...
// 原生App支付签名: 自动区分callback或promise
Payment.prototype.getAppPayParams = function(params, callback) {
	var that = this;
	// callback模式
	if (typeof callback == 'function') return that._getAppPayParams(params, callback);
	// promise模式
	return new Promise(function(resolve, reject) {
		that._getAppPayParams(params, function(err, result) {
...
// 原生App支付签名(原始方法)
Payment.prototype._getAppPayParams = function(params, callback) {
	var that = this;
...
/* 支付结果通知 • 中间件 */
// Koa2
Payment.prototype.middlewareForKoa2 = function (notifyUrl) {
  var that = this;
	return async function (ctx, next) {
    if (notifyUrl !== ctx.req.url) return next()
		if (ctx.method != 'POST') throw new Error('请求来源错误');
    ctx.weixin = {} // 用于判断微信通知接口
...
Payment.prototype.success = function(){
  return util.buildXML({
    return_code: 'SUCCESS'
  });
};

Payment.prototype.fail = function(msg){
  return util.buildXML({
    return_code: 'FAIL',
    return_msg: msg
  });
